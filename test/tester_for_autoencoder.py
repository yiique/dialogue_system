...
class TestModel(graph_base.GraphBase):
    ...
    def build_tower(self):
        ...
        prob_simple = self.train_forward_simple(src_dialogue, src_mask)
        train_loss_simple = -tf.reduce_mean(
            tf.reduce_sum(tf.reduce_sum(
                tf.one_hot(tf.to_int32(src_dialogue), FLAGS.common_vocab + FLAGS.candidate_num, 1.0, 0.0) *
                tf.log(tf.clip_by_value(prob_simple, 1e-20, 1.0)), -1) * src_mask, [0, 1])
        )
        train_grad_simple = self.optimizer.compute_gradients(train_loss_simple, self.params_simple)

        return src_dialogue, src_mask, train_loss_simple, train_grad_simple

    def train_forward_simple(self, src_dialogue, src_mask):
        ...
        _, _, _, prob_ta = control_flow_ops.while_loop(
            cond=lambda i, _1, _2, _3: i < src_index_ta.size(),
            body=self._train_step_simple,
            loop_vars=(
                tf.constant(0, tf.int32),
                src_index_ta, src_mask_ta, prob_ta
            )
        )
        return prob_ta.stack()

    def _train_step_simple(self, i, src_index_ta, src_mask_ta, prob_ta):
        ...
        prob = self.aux_decoder.forward(src_emb, tf.expand_dims(src_mask, -1), src_utterance)

        prob_ta = prob_ta.write(i, prob)

        return i+1, src_index_ta, src_mask_ta, prob_ta
...
def main():
    ...
    with tf.device('/gpu:0'):
        ...
        s_d, s_m, loss, grad = model.build_tower()
