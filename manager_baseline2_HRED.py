...
def train_iter(ep, sess, model, tower_records,
               tower_losses, avg_losses_decoder, update):
    count = 0
    losses = []
    ...
    while True:
        ...
        for i in range(FLAGS.dia_max_len):
            ...
            for j in range(FLAGS.GPU_num):
                ...
                feed_dict[tower_records[j][4]] = turn_mask
                feed_dict[tower_records[j][5]] = hred_hiddens[j]
                feed_dict[tower_records[j][6]] = hred_memorys[j]

            outputs = sess.run([tower_losses, avg_losses_decoder, update], feed_dict=feed_dict)
            ...
            with tf.device('/gpu:%d' % gpu_id):
                ...
                with tf.name_scope('tower_%d' % gpu_id):
                    with tf.variable_scope('cpu_variables', reuse=gpu_id > 0):
                        t_src, t_src_mask, t_tgt_indices, t_tgt_mask, t_turn_mask, \
                        t_hidden, t_memory, \
                        ...
                        tower_records.append(
                            (t_src, t_src_mask, t_tgt_indices, t_tgt_mask, t_turn_mask,
                             ...
                             hidden_t, memory_t))
        _, _, _, _, _, _, _, \
        tower_losses_decoder, tower_grads,\
